...
static int btusb_setup_csr(struct hci_dev *hdev)
{
	...
	if (is_fake) {
		...
		set_bit(HCI_QUIRK_BROKEN_READ_VOICE_SETTING, &hdev->quirk_flags);
		set_bit(HCI_QUIRK_BROKEN_READ_PAGE_SCAN_TYPE, &hdev->quirk_flags);
		       ...
		       const struct usb_device_id *id)
{
	...
	if (id->driver_info & BTUSB_ACTIONS_SEMI) {
		...
		set_bit(HCI_QUIRK_BROKEN_READ_ENC_KEY_SIZE, &hdev->quirk_flags);
		set_bit(HCI_QUIRK_BROKEN_EXT_CREATE_CONN, &hdev->quirk_flags);
		set_bit(HCI_QUIRK_BROKEN_WRITE_AUTH_PAYLOAD_TIMEOUT, &hdev->quirk_flags);
	}
